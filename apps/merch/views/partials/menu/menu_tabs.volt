...
<a class="menu-icons menu-region" tabindex="-1" data-code="{{ tab['name_en'] }}" href="{{ uriBase }}/{{ tab['name'] }}"> {{ tab['name'] }} <b class="menu-glyphicon visible-xs visible-sm glyphicon glyphicon-plus"></b> <span class="hidden-xs hidden-sm caret"></span> </a>
...
{% if not(tab is empty) %}
	...
	<a class="menu-country" data-code="{{ tabSub['country_code'] }}" tabindex="-1" href="{{ uriBase }}/{{ tab['name'] }}/{{ tabSub['name']  }}">{{ tabSub['name'] }} </a>
	...
	<script type="text/javascript">availCity['{{ tabSubmenu["city_name_en"] }}'] = '{{ tabSubmenu["name"] }}';</script>
	{% endif %}
	{% endfor %}
	{% endif %}
